...
- **Sliding cookie**: `GuestSessionInterceptor` re-issues `acme_session_id` on every request to a cart endpoint (`/api/v1/carts/**`) from a guest with a valid session, before the handler runs, so a 400 is covered too. It records cart activity at the same time (PIN-287, PIN-288).
