 ...
 * reaching the test body already means both succeeded. That covers all seven entities
 * whether or not they are named below, so the `count()` calls are documentation — they
 * list the mapped tables and confirm each is queryable.
 *
 ...
 *
 * Nor does it cover `event_store` or `customer_number_sequences`: those have no `@Entity`
 * and are reached through hand-written `JdbcTemplate` SQL, so drift there still only
 * breaks at runtime.
 */
@DataJpaTest
