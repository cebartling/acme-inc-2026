...
class CustomerEventPublisher(
    ...
    /**
     * Publishes a CustomerActivated event to Kafka.
     *
     * The event is serialized to JSON and published synchronously within
     * the transaction to ensure that failures prevent transaction commit.
     * The aggregate ID (customer ID) is used as the message key to
     * ensure ordering of events for the same customer.
     *
     * @param event The CustomerActivated event to publish.
     * @throws RuntimeException if publishing fails, causing transaction rollback.
     */
    fun publish(event: CustomerActivated) {
        ...
        logger.debug(
            ...
            event.eventId,
            event.payload.customerId,
            CustomerActivated.TOPIC
        )

        try {
            val sendResult = kafkaTemplate.send(CustomerActivated.TOPIC, key, value)
                .get(publishTimeoutSeconds, TimeUnit.SECONDS)
            logger.info(
                "Published {} event {} for customer {} to topic {} partition {} offset {}",
                event.eventType,
    ...
    /**
     * Publishes a PreferencesUpdated event to Kafka.
     *
     * The event is serialized to JSON and published synchronously within
     * the transaction to ensure that failures prevent transaction commit.
     * The aggregate ID (customer ID) is used as the message key to
     * ensure ordering of events for the same customer.
     *
     * @param event The PreferencesUpdated event to publish.
     * @throws RuntimeException if publishing fails, causing transaction rollback.
     */
    fun publish(event: PreferencesUpdated) {
        val key = event.aggregateId.toString()
        val value = objectMapper.writeValueAsString(event)
        ...
        logger.debug(
            ...
            event.eventId,
            event.payload.customerId,
            PreferencesUpdated.TOPIC
        )

        try {
            val sendResult = kafkaTemplate.send(PreferencesUpdated.TOPIC, key, value)
                .get(publishTimeoutSeconds, TimeUnit.SECONDS)
            logger.info(
                "Published {} event {} for customer {} to topic {} partition {} offset {}",
                event.eventType,
