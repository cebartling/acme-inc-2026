...
async function issueResetToken(world: CustomWorld, expired: boolean): Promise<string> {
...
Then('I should see a {string} confirmation', async function (this: CustomWorld, snippet: string) {
...
Then(
  ...
  async function (this: CustomWorld) {
    await expect(this.page.getByTestId('forgot-password-sent')).toContainText(/expires in 1 hour/i);
  }
);

Given('I am on the reset-password page with an expired token', async function (this: CustomWorld) {
...
Then('I should see an {string} message', async function (this: CustomWorld, _description: string) {
...
Then('I should see a link to request a new reset link', async function (this: CustomWorld) {
...
Given('a valid password reset token exists for an active user', async function (this: CustomWorld) {
  await issueResetToken(this, false);
});

Given('I am on the reset-password page with that token', async function (this: CustomWorld) {
...
Then(
  ...
  async function (this: CustomWorld) {
    await expect(this.page.getByTestId('reset-password-error')).toContainText(/do not match/i, {
      timeout: 5000,
    });
  }
);

Then('the reset-password form should still be visible', async function (this: CustomWorld) {
  await expect(this.page.getByTestId('reset-password-form')).toBeVisible();
});

Then('I should see the password requirements list', async function (this: CustomWorld) {
  await expect(this.page.getByTestId('reset-password-requirements')).toBeVisible({
    timeout: 10000,
  });
});

Then('at least one requirement should be marked as unmet', async function (this: CustomWorld) {
