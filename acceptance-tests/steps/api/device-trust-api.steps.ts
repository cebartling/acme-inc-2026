...
Given('the user {string} has TOTP MFA enabled', async function (this: CustomWorld, email: string) {
  ...
  const response = await this.identityApiClient.post(`/api/v1/test/users/${userId}/enable-mfa`, {
    totpSecret,
  });
...
Given(
  'the user has a valid device trust from a previous signin',
...
Given(
  'the user has a valid device trust with fingerprint {string}',
  async function (this: CustomWorld, fingerprint: string) {
    const userId = this.getTestData<string>('testUserId');
...
Given(
  'the user has a valid device trust with user agent {string}',
  async function (this: CustomWorld, userAgent: string) {
    const userId = this.getTestData<string>('testUserId');
...
Given(
  'the user has a device trust with ID {string}',
  async function (this: CustomWorld, deviceTrustId: string) {
    const userId = this.getTestData<string>('testUserId');
...
Given(
  'the user {string} has a device trust with ID {string}',
  async function (this: CustomWorld, email: string, deviceTrustId: string) {
    const userId = this.getTestData<string>(`testUserId_${email}`);
...
Given(
  'the user has a valid device trust created {int} days ago',
  async function (this: CustomWorld, daysAgo: number) {
    const userId = this.getTestData<string>('testUserId');
...
Given(
  'the user has a valid device trust created from IP {string}',
  async function (this: CustomWorld, ipAddress: string) {
    const userId = this.getTestData<string>('testUserId');
...
Given(
  'I have an active session for {string} with device trust {string}',
  async function (this: CustomWorld, email: string, deviceTrustId: string) {
    ...
    if (signinResponse.data.status === 'MFA_REQUIRED') {
      const mfaToken = signinResponse.data.mfaToken;
      const totpSecret =
        this.getTestData<string>('totpSecret') || 'JBSWY3DPEHPK3PXPJBSWY3DPEHPK3PXP';
      const totpCode = generateSync({ secret: totpSecret });
...
When(
  'I verify MFA with the correct TOTP code and rememberDevice set to true',
  async function (this: CustomWorld) {
...
When(
  'I verify MFA with the correct TOTP code and rememberDevice set to false',
  async function (this: CustomWorld) {
    const mfaToken = this.getTestData<string>('mfaToken');
    const totpSecret = this.getTestData<string>('totpSecret') || 'JBSWY3DPEHPK3PXPJBSWY3DPEHPK3PXP';
    ...
    const response = await this.identityApiClient.post('/api/v1/auth/mfa/verify', {
      mfaToken,
      code: totpCode,
      method: 'TOTP',
      rememberDevice: false,
...
When(
  'I signin with email {string} and password {string} with the device trust cookie',
...
When(
  'I signin with email {string} and password {string} with the expired device trust cookie',
  async function (this: CustomWorld, email: string, password: string) {
    const actualEmail = this.getTestData<string>('testUserEmail') || email;
    const deviceTrustId = this.getTestData<string>('deviceTrustId');
    const deviceFingerprint = this.getTestData<string>('deviceFingerprint');
...
When(
  'I signin with email {string} and password {string} from a new device',
  async function (this: CustomWorld, email: string, password: string) {
    const actualEmail = this.getTestData<string>('testUserEmail') || email;
...
When(
  'I signin with email {string} and password {string} with fingerprint {string}',
  async function (this: CustomWorld, email: string, password: string, fingerprint: string) {
    const actualEmail = this.getTestData<string>('testUserEmail') || email;
    const deviceTrustId = this.getTestData<string>('deviceTrustId');
...
When(
  'I signin with email {string} and password {string} with user agent {string}',
  async function (this: CustomWorld, email: string, password: string, userAgent: string) {
    const actualEmail = this.getTestData<string>('testUserEmail') || email;
    const deviceTrustId = this.getTestData<string>('deviceTrustId');
...
When(
  'I signin with email {string} and password {string} from IP {string}',
  async function (this: CustomWorld, email: string, password: string, ipAddress: string) {
    const actualEmail = this.getTestData<string>('testUserEmail') || email;
    const deviceTrustId = this.getTestData<string>('deviceTrustId');
    const deviceFingerprint = this.getTestData<string>('deviceFingerprint');
    const userAgent = this.getTestData<string>('deviceUserAgent');
...
When(
  'I signin with email {string} and password {string}',
  async function (this: CustomWorld, email: string, password: string) {
    const userAgent =
      this.getTestData<string>('testUserAgent') ||
...
When(
  'I request GET {string} without authentication',
...
When(
  ...
  async function (this: CustomWorld, oldPassword: string, newPassword: string) {
    const accessToken = this.getTestData<string>('access_token');
...
Then(
  'the response should not set a cookie named {string}',
  function (this: CustomWorld, cookieName: string) {
    const response = this.getTestData<{ status: number; headers: Record<string, unknown> }>(
      'lastResponse'
    );
...
Then(
  'the {word} cookie should have HttpOnly flag',
  function (this: CustomWorld, cookieName: string) {
...
Then(
  'the {word} cookie should have SameSite=Strict',
  function (this: CustomWorld, cookieName: string) {
    const response = this.getTestData<{ headers: Record<string, unknown> }>('lastResponse');
    const setCookieHeader = response!.headers['set-cookie'];
...
Then(
  'the {word} cookie should have Path={string}',
  function (this: CustomWorld, cookieName: string, expectedPath: string) {
    const response = this.getTestData<{ headers: Record<string, unknown> }>('lastResponse');
    const setCookieHeader = response!.headers['set-cookie'];
...
Then(
  'the {word} cookie should have Max-Age={int}',
  function (this: CustomWorld, cookieName: string, expectedMaxAge: number) {
    const response = this.getTestData<{ headers: Record<string, unknown> }>('lastResponse');
    const setCookieHeader = response!.headers['set-cookie'];
...
Then('the device trust token should be stored in Redis', async function (this: CustomWorld) {
  ...
  const response = await this.identityApiClient.get(
    `/api/v1/test/device-trusts/${deviceTrustToken}`
  );
...
Then(
  'the device trust should have a TTL of approximately {int} seconds',
  async function (this: CustomWorld, expectedTtl: number) {
    ...
    const response = await this.identityApiClient.get(
      `/api/v1/test/device-trusts/${deviceTrustToken}/ttl`
    );
...
Then(
  'the user should have exactly {int} device trusts in Redis',
...
Then(
  'the user should have {int} device trusts in Redis',
  async function (this: CustomWorld, expectedCount: number) {
    const userId = this.getTestData<string>('testUserId');
...
Then('the oldest device trust should have been evicted', async function (this: CustomWorld) {
  ...
  if (oldDeviceTrustIds.length > 0) {
    ...
    const response = await this.identityApiClient.get(
      `/api/v1/test/device-trusts/${oldestDeviceTrustId}`
    );
...
Then(
  'a DeviceRevoked event should be published with reason {word}',
  async function (this: CustomWorld, reason: string) {
    ...
    const response = await this.identityApiClient.get(
      `/api/v1/test/events/DeviceRevoked?userId=${userId}`
    );
...
Then(
  '{int} DeviceRevoked events should be published with reason {word}',
  async function (this: CustomWorld, count: number, reason: string) {
    const userId = this.getTestData<string>('testUserId');
    ...
    const eventsWithReason = response.data.events.filter(
      (e: { payload: { reason: string } }) => e.payload.reason === reason
    );
...
Then('a DeviceRemembered event should be published', async function (this: CustomWorld) {
  ...
  const response = await this.identityApiClient.get(
    `/api/v1/test/events/DeviceRemembered?userId=${userId}`
  );
  ...
  this.setTestData(
    'latestDeviceRememberedEvent',
    response.data.events[response.data.events.length - 1]
  );
});

Then(
  'the DeviceRemembered event should include {word}',
  function (this: CustomWorld, field: string) {
    const event = this.getTestData<{ payload: Record<string, unknown> }>(
      'latestDeviceRememberedEvent'
    );
...
Then(
  'the response should contain exactly {int} devices',
...
Then('each device should include {word}', function (this: CustomWorld, field: string) {
  const response = this.getTestData<{ data: { devices: Record<string, unknown>[] } }>(
    'lastResponse'
  );
...
Then(
  'the device trust {string} should not exist in Redis',
  async function (this: CustomWorld, deviceTrustId: string) {
    const response = await this.identityApiClient.get(
      `/api/v1/test/device-trusts/${deviceTrustId}`
    );
...
Then(
  'the device trust {string} should still exist in Redis',
  async function (this: CustomWorld, deviceTrustId: string) {
    const response = await this.identityApiClient.get(
      `/api/v1/test/device-trusts/${deviceTrustId}`
    );
...
Then(
  'the first device should have deviceName {string}',
...
Then(
  'the device {string} should have isCurrent set to true',
  function (this: CustomWorld, deviceTrustId: string) {
    const response = this.getTestData<{ data: { devices: DeviceTrustInfo[] } }>('lastResponse');
...
Then(
  'the response should contain {string} status',
...
Then('the password change should succeed', function (this: CustomWorld) {
  ...
  if (success === false) {
    throw new Error(
      'Password change endpoint returned an error. Feature may not be implemented yet.'
    );
  }
