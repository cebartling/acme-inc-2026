...
Given('the user has {int} active sessions', async function (this: CustomWorld, sessionCount: number) {
  const userId = this.getTestData<string>('testUserId');
  ...
  if (response.status !== 200) {
    ...
    throw new Error(`Failed to create sessions: ${response.status} - ${JSON.stringify(response.data)}`);
  }

  this.setTestData('existingSessionCount', sessionCount);
});

Given('the user has {int} active sessions from different devices', async function (this: CustomWorld, sessionCount: number) {
  const userId = this.getTestData<string>('testUserId');
...
When('I complete signin and MFA verification for {string}', async function (this: CustomWorld, email: string) {
...
When('I complete signin and SMS MFA verification for {string}', async function (this: CustomWorld, email: string) {
  // Use the actual email that was created (may be unique)
  const actualEmail = this.getTestData<string>('testUserEmail') || email;
  const password = this.getTestData<string>('testUserPassword') || 'ValidP@ss123!';
...
When('I measure the response time for {int} MFA verification requests', async function (this: CustomWorld, requestCount: number) {
  const password = this.getTestData<string>('testUserPassword') || 'ValidP@ss123!';
  const totpSecret = this.getTestData<string>('totpSecret');
  const email = this.getTestData<string>('testUserEmail');

  if (!totpSecret) {
    throw new Error('TOTP secret must be set before running performance test. Current value: ' + totpSecret);
  }
  if (!email) {
...
Then('the response should set a secure cookie named {string}', function (this: CustomWorld, cookieName: string) {
...
Then('the {word} token cookie should have HttpOnly flag', function (this: CustomWorld, tokenType: string) {
...
Then('the {word} token cookie should have Secure flag', function (this: CustomWorld, tokenType: string) {
  const cookieName = `${tokenType.toLowerCase()}_token`;
  const setCookieHeaders = this.getTestData<string[]>('setCookieHeaders');
...
Then('the {word} token cookie should have SameSite=Strict', function (this: CustomWorld, tokenType: string) {
  const cookieName = `${tokenType.toLowerCase()}_token`;
  const setCookieHeaders = this.getTestData<string[]>('setCookieHeaders');
...
Then('the {word} token cookie should have Path={string}', function (this: CustomWorld, tokenType: string, path: string) {
  const cookieName = `${tokenType.toLowerCase()}_token`;
  const setCookieHeaders = this.getTestData<string[]>('setCookieHeaders');
...
Then('the {word} token cookie should have Max-Age={int}', function (this: CustomWorld, tokenType: string, maxAge: number) {
  const cookieName = `${tokenType.toLowerCase()}_token`;
  const setCookieHeaders = this.getTestData<string[]>('setCookieHeaders');
...
Then('the response should set exactly {int} cookies', function (this: CustomWorld, expectedCount: number) {
  const setCookieHeaders = this.getTestData<string[]>('setCookieHeaders');
...
Then('the {word} token JWT should include a {string} header', function (this: CustomWorld, tokenType: string, headerName: string) {
  const cookieName = `${tokenType.toLowerCase()}_token`;
...
Then('the {string} header should match pattern {string}', function (this: CustomWorld, headerName: string, pattern: string) {
  const header = this.getTestData<JWTHeader>('access_jwt_header') || this.getTestData<JWTHeader>('refresh_jwt_header');
...
Then('the {word} token JWT should include {string} claim', function (this: CustomWorld, tokenType: string, claimName: string) {
  const cookieName = `${tokenType.toLowerCase()}_token`;
  const tokenValue = this.getTestData<string>(`${cookieName}_value`);
...
Then('the {word} token JWT should include claim {string} with the user\'s UUID', function (this: CustomWorld, tokenType: string, claimName: string) {
  const cookieName = `${tokenType.toLowerCase()}_token`;
  const tokenValue = this.getTestData<string>(`${cookieName}_value`);
  const userId = this.getTestData<string>('testUserId');
...
Then('the {word} token JWT should include claim {string} with value {string}', function (this: CustomWorld, tokenType: string, claimName: string, expectedValue: string) {
  const cookieName = `${tokenType.toLowerCase()}_token`;
  const tokenValue = this.getTestData<string>(`${cookieName}_value`);
...
Then('the {word} token JWT should not include claim {string}', function (this: CustomWorld, tokenType: string, claimName: string) {
  const cookieName = `${tokenType.toLowerCase()}_token`;
  const tokenValue = this.getTestData<string>(`${cookieName}_value`);
...
Then('the {word} token JWT header should have {string} value {string}', function (this: CustomWorld, tokenType: string, headerName: string, expectedValue: string) {
  const cookieName = `${tokenType.toLowerCase()}_token`;
  const tokenValue = this.getTestData<string>(`${cookieName}_value`);
...
Then('the tokenFamily should match pattern {string}', function (this: CustomWorld, pattern: string) {
...
Then('the session TTL in Redis should be approximately {int} seconds', async function (this: CustomWorld, expectedTtl: number) {
  const userId = this.getTestData<string>('testUserId');
  const session = this.getTestData<SessionData>('latestSession');
  ...
  const response = await this.identityApiClient.get(`/api/v1/test/sessions/${session.sessionId}/ttl`);
...
Then('the user should have exactly {int} active sessions', async function (this: CustomWorld, expectedCount: number) {
  const userId = this.getTestData<string>('testUserId');
...
Then('a SessionCreated event should be published to Kafka', async function (this: CustomWorld) {
  ...
  const response = await this.identityApiClient.get(`/api/v1/test/events/SessionCreated?userId=${userId}`);
...
Then('the event should contain {string} with value true', function (this: CustomWorld, fieldName: string) {
  const event = this.getTestData('latestSessionCreatedEvent') ||
                this.getTestData('latestUserLoggedInEvent') ||
                this.getTestData('latestSessionInvalidatedEvent');
...
Then('the invalidated session reason should be {string}', function (this: CustomWorld, expectedReason: string) {
...
Then('the response body should not contain {string}', function (this: CustomWorld, searchString: string) {
...
Then('the application logs should not contain any JWT token strings', async function (this: CustomWorld) {
...
Then('the 95th percentile response time should be less than {int}ms', function (this: CustomWorld, maxResponseTime: number) {
